...
//						        console.log(obj.item.length);
						    	...
						    	for (i = 0; i< obj.item.length;i++)
						    	{
						    		...
						    		var itemBtn = item.buttons[0].button;
						    		for (j = 0;j < itemBtn.length;j++)
						    		{
						    			var btn = itemBtn[j];
//						    			console.log(btn._);
                                        var objeto = btn.$;
...
//                                        console.log(objeto);
                                        ...
                                        xmlButtom.code = objeto.code;
                                        if(objeto.warning)
                                        {
                                            xmlButtom.warning = objeto.warning;
                                        }
                                        xmlButtom.descricao = btn._;
                                        xmlButtom.save(function(erre,saveXMLButtom){
//                                            console.log(err);
                                            if(!erre)
                                            {
//                                                console.log(saveXMLButtom);
                                            }
                                        });
						    		}
...
//						    		console.log(item);
						    		...
						    		xml.save(function(err,saveXML){
						    			...
						    			else
						    			{
						    				...
						    				retorno.mensagem = "XML com o seguinte erro na linha "+ i + ": "+err;
						    				res.json(retorno);
						    			}
						    		});
